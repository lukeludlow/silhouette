...
public class CyclingControl<T extends Enum<T>> implements Control<T> {
    private final OptionUi<T> option;
    private final T[] allowedValues;
    private final Text[] names;

    public CyclingControl(OptionUi<T> option, Class<T> enumType) {
        this(option, enumType, enumType.getEnumConstants());
    }

    public CyclingControl(OptionUi<T> option, Class<T> enumType, Text[] names) {
    ...
    public CyclingControl(OptionUi<T> option, Class<T> enumType, T[] allowedValues) {
        T[] universe = enumType.getEnumConstants();
    ...
    private static class CyclingControlElement<T extends Enum<T>> extends ControlElement<T> {
        ...
        public CyclingControlElement(OptionUi<T> option, Dim2i dim, T[] allowedValues, Text[] names) {
            super(option, dim);
